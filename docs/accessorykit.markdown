...
* <b>[NSAccessorySetupKitSupports](https://developer.apple.com/documentation/bundleresources/information-property-list/nsaccessorysetupkitsupports):</b> An array of strings that indicates the wireless technologies AccessorySetupKit uses when discovering and configuring accessories.
* <b>[NSAccessorySetupBluetoothServices](https://developer.apple.com/documentation/bundleresources/information-property-list/nsaccessorysetupbluetoothservices):</b> An array of strings that represent the hexadecimal values of Bluetooth SIG-defined services or custom services for accessories your app configures.
* <b>[NSAccessorySetupBluetoothNames](https://developer.apple.com/documentation/bundleresources/information-property-list/nsaccessorysetupbluetoothnames):</b> An array of strings that represent the Bluetooth device names or substrings for accessories that your app configures.

See Apple’s [AccessorySetupKit documentation](https://developer.apple.com/documentation/accessorysetupkit) for the full rules these entries must follow.
...
```js
    const supported = BleManager.ACCESSORY_KIT_SUPPORTED; // boolean constant
```
...
```js
    ...
    async function getAccessoriesKitDevices() {
        const supported = BleManager.ACCESSORY_KIT_SUPPORTED;
        if (supported) {
            let accessories = await BleManager.getPairedAccessories();
...
AccessoryKit only handles discovery/pairing — it does **not** open a BLE connection.
The accessory `id` is its CoreBluetooth identifier, so before using an accessory you
should check whether it is already connected and, if not, connect to it first. Only
then continue with the normal usage (`retrieveServices`, read/write, notifications…).

```js
    async function connectAccessory(accessory) {
        const isConnected = await BleManager.isPeripheralConnected(accessory.id);
        if (!isConnected) {
            await BleManager.connect(accessory.id);
        }
        await BleManager.retrieveServices(accessory.id);
        // The accessory is ready to use (read / write / startNotification ...).
    }
```

### Picker setup options

Each display item accepts an optional `setupOptions` that maps to
[`ASPickerDisplayItem`](https://developer.apple.com/documentation/accessorysetupkit/aspickerdisplayitem):

* <b>setupOptions:</b> `"rename"` (let the user rename during setup) and/or `"confirmAuthorization"` (require an explicit confirmation step).
    ...
    await BleManager.accessoriesScan([
        {
            ...
            productImage: 'heart.fill',
            serviceUUID: '0000180D-0000-1000-8000-00805F9B34FB',
            setupOptions: ['rename', 'confirmAuthorization'],
        },
    ]);
```

### Removing and renaming accessories

```js
    // Forget a paired accessory and delete its BLE bond.
    await BleManager.removeAccessory(accessory.id);

    // Present the system sheet to rename a paired accessory.
    await BleManager.renameAccessory(accessory.id);
```

The `id` is the value returned by `accessoriesScan` / `getPairedAccessories`.
Both reject with `ACCESSORY_NOT_FOUND` when no paired accessory matches the id.
